...
template <typename T>
  requires roco::core::collections::is_iterator<T>
T::t_ref iterator_func(T &it) {
  return it.get();
}

template <typename T>
  requires roco::core::collections::is_iterator_back<T>
...
template <typename T>
  requires roco::core::collections::is_iterator_randacc<T>
...
int main() {
  ...
  { // iterator concepts
    ...
    int32_t &value = iterator_func<array_it<int32_t, 10, heap, false>>(it1);
    assert(value == 1);

    int32_t &value1 =
        iterator_back_func<array_it<int32_t, 10, heap, false>>(it1);
    assert(value1 == 0);

    int32_t &value2 =
        iterator_randacc_func<array_it<int32_t, 10, heap, false>>(it1);
    assert(value2 == 2);
  }
  { // const
    ...
    array_it<int32_t, 10, heap, true> it1 = arr.to_array_it_beg_const();
    array_it<int32_t, 10, heap, true> it2 = arr.to_array_it_end_const();

    for (size_t i = 0; it1 != it2; i++, it1.inc()) {
      assert(*it1 == i);
    }

    array_it<int32_t, 10, heap, true> it3 = arr.to_array_it_beg_const();
    array_it<int32_t, 10, heap, true> it4 = arr.to_array_it_end_const();
  ...
  { // const
    ...
    assert(!res.has_error());
    const array<int32_t, 10, heap> arr = res.take_value();
