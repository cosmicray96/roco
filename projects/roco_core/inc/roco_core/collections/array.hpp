...
public:
  array(size_t capacity, allocators::allocator *allocator)
      : m_data((t_elem *)(allocator->alloc(capacity * sizeof(t_elem)))),
...
public:
  ...
  array_iterator<t_elem> end() {
    return array_iterator<t_elem>(m_data + m_count);
  }
...
public:
  static array<t_elem> copy(const array<t_elem> &other,
                            allocators::allocator *a) {
    array<t_elem> new_array(other.m_capacity, a);
    new_array.m_count = other.m_count;
    memcpy(new_array.m_data, other.m_data,
           new_array.m_capacity * sizeof(t_elem));
    return new_array;
  }
...
public:
  void inc() { m_ptr++; }
  bool equals(const array_iterator<t_elem> &other) {
