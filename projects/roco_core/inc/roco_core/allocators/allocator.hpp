...
concept is_allocator = requires(A a, size_t size, size_t align, void *ptr) {
    { A::alloc(size, align) } -> std::same_as<roco::core::result<void *, roco::core::error_enum>>;
    { A::alloc8(size) } -> std::same_as<roco::core::result<void *, roco::core::error_enum>>;
    { A::alloc16(size) } -> std::same_as<roco::core::result<void *, roco::core::error_enum>>;
...
result<T *, roco::core::error_enum> alloc_type(Args &&...args) {
    ...
    T *output =
        std::construct_at(reinterpret_cast<T *>(res.take_value()), std::forward<Args>(args)...);
    if (!output) {
...
template <typename A, typename T, typename... Args>
    requires is_allocator<A>
result<T *, roco::core::error_enum> alloc_type_array(size_t count, Args &&...args) {
    if (count == 0) {
        return {roco::core::error_enum::alloc_array_bad_size};
    }
    size_t padded_size = aligned_size(sizeof(T) * count, alignof(T));
    result<void *, roco::core::error_enum> res = A::alloc(padded_size, alignof(T));
    if (res.has_error()) {
        return {res.take_error()};
    }
