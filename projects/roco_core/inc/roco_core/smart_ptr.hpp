...
public:
  ...
  uptr(uptr<U, A> &&other)
      : m_(static_cast<T *>(other.m_)), m_delete_fn(other.m_delete_fn) {
    other.m_ = nullptr;
...
public:
  ...
  friend result<uptr<_T, _A>, error_enum> uptr_take(_T *ptr);

  template <typename _T, typename _A, typename... _Args>
    requires allocators::is_allocator<_A>
  friend result<uptr<_T, _A>, error_enum> uptr_make(_Args &&...args);
...
template <typename T, typename A>
  requires allocators::is_allocator<A>
result<uptr<T, A>, error_enum> uptr_take(T *ptr) {
  if (!ptr) {
    return {error_enum::is_null_pointer};
  }
  uptr<T, A> p;
  p.m_ = ptr;
  p.m_delete_fn = [](void *ptr) {
    allocators::dealloc_type<A>(static_cast<T *>(ptr));
  };
  return {p};
}
...
template <typename T, typename A, typename... Args>
  requires allocators::is_allocator<A>
result<uptr<T, A>, error_enum> uptr_make(Args &&...args) {
  uptr<T, A> ptr;
...
public:
  ...
  template <typename _T, typename _A, typename... _Args>
    requires allocators::is_allocator<_A>
  friend result<sptr<_T, _A>, error_enum> sptr_make(_Args &&...args);

  template <typename _T, typename _A>
    requires allocators::is_allocator<_A>
  friend result<sptr<_T, _A>, error_enum> sptr_take(_T *ptr);
...
template <typename T, typename A>
  requires allocators::is_allocator<A>
result<sptr<T, A>, error_enum> sptr_take(T *ptr) {
  if (!ptr) {
    return {error_enum::is_null_pointer};
  }
  sptr<T, A> p;
  result<sptr_ctx<A> *, error_enum> res =
      allocators::alloc_type<A, sptr_ctx<A>>();
  if (res.has_error()) {
    return {res.take_error()};
  }
